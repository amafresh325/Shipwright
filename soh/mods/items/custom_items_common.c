...
// Sicherstellen, dass das DL bekannt ist
extern Gfx gHyliaGraceGiveDL[];
extern Gfx gDemiseDestructionGiveDL[];
extern Gfx gZonaiPermafrostGiveDL[];

// Deine "minimale" Funktion
void DrawCustomExtChampionsTunic(PlayState* play, GetItemEntry* getItemEntry) {
    // Lichtquellen optimiert: Sehr helles Hauptlicht von vorne, damit die gelben Kreuze richtig strahlen
    static Lights1 brightLights = gdSPDefLights1(
        40, 75, 110,   // Ambient (Schatten) -> Kühles, tiefblaues Grundleuchten in den Falten
        245, 245, 255, // Diffuse (Hauptlicht) -> Knalliges Licht von vorne, bringt die Details perfekt raus
    ...
    // 1. FARBEN FÜR DEN STOFF (Dein originales, helles Babyblau/Hellblau)
    u8 targetR = 82 + pulse;
    u8 targetG = 163 + pulse;
    u8 targetB = 197 + pulse;

    // 2. FARBEN FÜR DEN KRAGEN (Ein deutlich dunkleres, satteres Dunkelblau)
    u8 collarR = 30 + (pulse / 2);
    u8 collarG = 85 + (pulse / 2);
    u8 collarB = 135 + (pulse / 2);

    // A) KRAGEN ZEICHNEN: Direkt im edlen Dunkelblau (Kein Grayscale -> Sicherer Vanilla-Support!)
    ...
    // B) STOFF & KREUZE ZEICHNEN: Umschalten auf das hellere, pulsierende Champions-Blau
...
void DrawCustomExtSpiritBreastplate(PlayState* play, GetItemEntry* getItemEntry) {
    // Lichtquellen optimiert: Warmes Gold im Schatten, aber ein knallweißes Hauptlicht,
    // damit die Kreuze/Schnüre auf der Brust einen weißlichen Glanz bekommen!
    static Lights1 brightLights = gdSPDefLights1(
        150, 100, 15,   // Ambient (Schatten) -> Warmes Gold in den Falten
        255, 255, 255,  // Diffuse (Hauptlicht) -> Pures Weiß, bleicht die Kreuze Richtung Weiß aus
        0, 80, 80       // Lichtrichtung
    );
    ...
    // 1. FARBEN FÜR DIE RÜSTUNG (Dein originales, glühendes Gold)
    u8 targetR = 225 + pulse;       // Variiert zwischen 225 und 255
    u8 targetG = 180 + pulse;       // Variiert zwischen 180 und 210
    u8 targetB = 30 + (pulse / 2);  // Variiert zwischen 30 und 45

    // 2. FARBEN FÜR DEN KRAGEN (Ein tieferes, dunkleres Altgold/Bronze)
    u8 collarR = 160 + pulse;       // Deutlich weniger Rot- und Grünanteil für einen dunkleren Ton
    u8 collarG = 115 + pulse;
    u8 collarB = 10 + (pulse / 2);

    // A) KRAGEN ZEICHNEN: Im dunkleren Gold-Farbton (Sicherer Vanilla-Support ohne Grayscale)
    gDPSetPrimColor(POLY_OPA_DISP++, 0, 0, collarR, collarG, collarB, 255);
    gDPSetEnvColor(POLY_OPA_DISP++, collarR / 2, collarG / 2, collarB / 2, 255);
    gSPDisplayList(POLY_OPA_DISP++, (Gfx*)gGiTunicCollarDL);
    
    // B) RÜSTUNG & KREUZE ZEICHNEN: Umschalten auf das hellere Hauptgold
    // Durch das reinweiße Diffuse-Licht heben sich die Kreuze jetzt hell/weißlich ab.
    gDPSetPrimColor(POLY_OPA_DISP++, 0, 0, targetR, targetG, targetB, 255);
    gDPSetEnvColor(POLY_OPA_DISP++, targetR / 2, targetG / 2, targetB / 2, 255); 
    gSPDisplayList(POLY_OPA_DISP++, (Gfx*)gGiTunicDL);
...
// Hier einfach nur die Deklaration (kein extern "C"!)
void DrawCustomHyliaGrace(PlayState* play, GetItemEntry* getItemEntry) {
    // 1. Gfx Context öffnen
    OPEN_DISPS(play->state.gfxCtx);

    // 2. Standard-Transparenz-Setup (für Kristall-Look)
    Gfx_SetupDL_25Xlu(play->state.gfxCtx);

    // 3. Matrix-Stack sichern
    Matrix_Push();

    // 4. Skalierung: Wenn er zu klein/groß ist, ändere die 1.0f hier!
    Matrix_Scale(1.0f, 1.0f, 1.0f, MTXMODE_APPLY);

    // 5. Matrix in die Grafik-Pipeline pushen
    gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(play->state.gfxCtx, __FILE__, __LINE__), 
              G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);

    // 6. Dein DL abfeuern
    gSPDisplayList(POLY_XLU_DISP++, gHyliaGraceGiveDL);

    // 7. Matrix-Stack aufräumen
    Matrix_Pop();

    // 8. Gfx Context schließen
    CLOSE_DISPS(play->state.gfxCtx);
}


void DrawCustomDemiseDestruction(PlayState* play, GetItemEntry* getItemEntry) {
    OPEN_DISPS(play->state.gfxCtx);
    Gfx_SetupDL_25Xlu(play->state.gfxCtx);
    Matrix_Push();
    Matrix_Scale(1.0f, 1.0f, 1.0f, MTXMODE_APPLY);
    gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(play->state.gfxCtx, __FILE__, __LINE__), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
    gSPDisplayList(POLY_XLU_DISP++, gDemiseDestructionGiveDL);
    Matrix_Pop();
    CLOSE_DISPS(play->state.gfxCtx);
}

void DrawCustomZonaiPermafrost(PlayState* play, GetItemEntry* getItemEntry) {
    OPEN_DISPS(play->state.gfxCtx);
    Gfx_SetupDL_25Xlu(play->state.gfxCtx);
    Matrix_Push();
    Matrix_Scale(1.0f, 1.0f, 1.0f, MTXMODE_APPLY);
    gSPMatrix(POLY_XLU_DISP++, Matrix_NewMtx(play->state.gfxCtx, __FILE__, __LINE__), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
    gSPDisplayList(POLY_XLU_DISP++, gZonaiPermafrostGiveDL);
    Matrix_Pop();
    CLOSE_DISPS(play->state.gfxCtx);
}
